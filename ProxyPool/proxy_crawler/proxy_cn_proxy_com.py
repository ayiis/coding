...
target_page_struct_list = config.crawler_setting["proxy.cn-proxy.com"]["target_page_struct_list"]
collection_name = config.crawler_setting["proxy.cn-proxy.com"]["collection_name"]
data_source = config.crawler_setting["proxy.cn-proxy.com"]["data_source"]
...
def grep_page_ip_list(page_html):
    retult = []
    ip_tables = page_html.xpath("//table[@class=\"sortable\"]")
    for ip_table in ip_tables:
        ip_trs = ip_table.xpath("tbody/tr")
        retult += [ [ td.xpath("string()").strip() for td in tr.xpath("td") ] for tr in ip_trs ]
...
def do(mongodb):
    ...
    print "Job proxy_cn_proxy_com start at %s!" % datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    print "Job proxy_cn_proxy_com done at %s!" % datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
