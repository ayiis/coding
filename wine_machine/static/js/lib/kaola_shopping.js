...
window.kaola_shopping = {
                    ...
                    for (var i = 0 ; i < res_data.data.length ; i++ ) {
                        ...
                        if (item["quan"] && item["quan"].length > 0) {
                            ...
                            for (var j = 0 ; j < item["quan"].length ; j++ ) {
                                quan_html.push('<span class="info_quan" title="' + item["quan"][j]['tipTitle'] + '">' + item["quan"][j]['tipTitle'] + '</span>');
                            }
                            name_html.push('<br/>');
                        ...
                        if (item["presale"]) {
                            ...
                            name_html.push('<br/><span class="info_presale" > 预售中 </span>');
                        }
                        ...
                        templete_task.find('td:eq(2)').find('a').attr('val', item.price).text(item.price).attr("href", "https://goods.kaola.com/product/" + item.itemid + ".html");
    ...
    update_status: function(_id, status) {
        ...
        $.ajax({
            type: 'POST',
            contentType: 'application/json; charset=utf-8',
            url: '/api/kaola/task_update_status',
            data: JSON.stringify(req_data),
            dataType: 'json',
            success: function(res_data) {
                if (res_data.code == 200) {
    ...
    update_good_price: function(_id, good_price) {
        ...
        $.ajax({
            type: 'POST',
            contentType: 'application/json; charset=utf-8',
            url: '/api/kaola/update_good_price',
            data: JSON.stringify(req_data),
            dataType: 'json',
            success: function(res_data) {
                if (res_data.code == 200) {
